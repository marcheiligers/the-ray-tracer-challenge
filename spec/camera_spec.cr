...
describe RayTracer::Camera do
  ...
  it "the pixel size for a horizontal canvas" do
    ...
    c.pixel_size.should be_close(0.01, Spec::DELTA)
  end
  ...
  it "constructing a ray through the center of the canvas" do
    ...
    r.direction.should be_close(RayTracer::Tuple.vector(0, 0, -1), Spec::DELTA)
  end
  ...
  it "constructing a ray through a corner of the canvas" do
    ...
    r.origin.should eq(RayTracer::Tuple.point(0, 0, 0))
    r.direction.should be_close(RayTracer::Tuple.vector(0.66519, 0.33259, -0.66851), Spec::DELTA)
  end
  ...
  it "constructing a ray when the camera is transformed" do
    ...
    sq2 = Math.sqrt(2) / 2
    r.direction.should be_close(RayTracer::Tuple.vector(sq2, 0, -sq2), Spec::DELTA)
  end
  ...
  it "constructing a ray when the camera is transformed" do
    ...
    image = c.render(w)
    image.pixel_at(5, 5).should be_close(RayTracer::Color.color(0.38066, 0.47583, 0.2855), Spec::DELTA)
  end
end
